$(document).ready(function () {
    let btnCheck = $('.btn_check_result'), resultType = $('#result_type'), result_typeSelectedValue, sessionSelect,
        codeInput = $('#code_input'), termSelect, body = $('body'), resultCheckForm, termSelectedValue,
        sessionSelectedValue, result, result_type;
...
//alert('now');
                        $(this).empty().html(`<i class="fa fa-spinner fa-spin fa-fw" style="margin-right: 10px;color:white;"></i>Checking Result...`);
                        setTimeout(function () {
    ...
    resultCheckForm.submit(function (e) {
        ...
        ajax.done((msg)=> {
            ...
            result_typeSelectedValue.val('');
            btnCheck.removeData('<i></i>');
            btnCheck.empty().html('Check Result');
            if (msg === 'false') {
            ...
            } else if (msg === 'Account deleted' || msg === 'Expelled') {
                ...
                $(`<div class="form-group col-11" id="check_option_container" style="display: flex;justify-content: space-evenly;align-items: center;">
            <button class="btn btn_summary process" type="button" id="summary">Summary</button>
            <button class="btn btn_full_detail process" type="button" id="full_detail">Full Detail</button>
        </div><div class="form-group" id="back_btn_container" style="margin-top:0;display: flex;justify-content: center;align-items: center">
            <button class="btn back process" type="button" id="back">Back</button></div>`).insertAfter('#check_btn_container');

                $('#summary').click(function () {

                    if (result_type === 'session-result') {
                        presentSessionSummerizedResult(msg);
                    } else {
                        presentTermSummerizedResult(msg);
                    }
                    ...
                    $('#full_detail').click(function () {
                        if (result_type === 'session-result') {
                            presentFullDetailedSessionResult(msg);
                        } else {
                            presentFullDetailedTermResult(msg);
                        }


                    });
                    $('#back').click(function () {
                        $('#summerized_table').remove();
                        $(this).hide();
                        $('#check_option_container').hide();
                        resultCheckForm.css('border', '2px solid black');
    ...
    function presentSessionSummerizedResult(resultdata) {
        let res;
        if (resultdata !== 'empty 2') {
            res = JSON.parse(resultdata);
        }

        resultCheckForm.css({'width': '40rem', 'border': 'none', 'marginTop': '20px'});
        $('#back').css('marginTop', '30px');

        //for(let n in res){
        //alert(res['name']);
        $(`<table id="summerized_table" style="margin: auto;overflow: auto"><tr>
                <td>Name</td><td>${res['name']}</td>
                <td>Status</td><td>${res['status']}</td>
                <td>Final Grade</td><td>${res['middle_row']['grade']}</td>
            </tr>
            <tr>
                <td>Final Position</td><td>${res['middle_row']['position']}</td>
                <td>Grand Total</td><td>${res['middle_row']['grandTotal']}</td>
                <td>Average</td><td>${res['middle_row']['average']}</td>
            </tr></table>`).insertAfter('#check_option_container');
        $('#summary').attr('disabled', 'disabled');
        //}

    }

    function presentTermSummerizedResult(resultdata) {
        let res;
        if (resultdata !== 'empty 2') {
            res = JSON.parse(resultdata);
        }
        ...
        $(`<table id="summerized_table" style="margin: auto;overflow: auto"><tr>
            ...
            <tr>
                <td>Final Position</td><td>${res['middle_row']['position']}</td>
                <td>Grand Total</td><td>${res['middle_row']['total']}</td>
                <td>Average</td><td>${res['middle_row']['average']}</td>
            </tr></table>`).insertAfter('#check_option_container');
        $('#summary').attr('disabled', 'disabled');
        //}
    ...
    <div class="result_header">
        <img src="../../../Backend/Src/Icons/${res['school_info']['school_logo']}" width="100" height="100" id="school_logo">
        <h2 id="school_name">${res['school_info']['schoolName']}</h2><img src="../../../Backend/Src/uploads/students/${res['passport']}" width="100" height="100" id="passport">
...
<tr style="text-align:center">
    ...
    function presentFullDetailedTermResult(resultdata) {
        ...
        $('.modal-body').html(function () {
            let partOne = `<div class="container" id="result_sheet" style="width: 97%;height: 95%">
    <div class="result_header">
        <img src="../../../Backend/Src/Icons/${res['school_info']['school_logo']}" width="100" height="100" id="school_logo">
        <h2 id="school_name">${res['school_info']['schoolName']}</h2><img src="../../../Backend/Src/uploads/students/${res['passport']}" width="100" height="100" id="passport">
        <h4 id="postal_address">${res['school_info']['schoolPostalBox']}</h4>
        <h5 id="school_motto">${res['school_info']['schoolMotto']}</h5>
        <div class="stud_details">
        <h5 style="text-decoration: underline;font-weight: bold;">Student Term Report Sheet</h5>
            <div class="row">
            <div class="side_bar_buttons">
                <b><p class="titles">Name: </p></b><p class="details">${res['name']}</p></div>
